...
class ApprovalWorkflowConfig(models.Model):
    """
    Master configuration for an approval workflow attached to a specific model.
    One config per model (enforced by unique constraint).
    """
    ...
    name = fields.Char(
        ...
        required=True,
        help='Descriptive name for this approval workflow.'
    )
    ...
    block_create = fields.Boolean(
        string='Block Create',
        default=False,
        help='Prevent creating new records until approved (rarely used).'
    )
    block_write = fields.Boolean(
        string='Block Write/Edit',
        default=True,
        help='Prevent editing records while approval is pending.'
    )
    block_unlink = fields.Boolean(
        string='Block Delete',
        default=True,
        help='Prevent deleting records while approval is pending.'
    )
    block_custom_actions = fields.Char(
        string='Block Custom Methods',
        help='Comma-separated list of model method names to block (e.g. action_confirm,action_validate).'
    )
    ...
    trigger_domain = fields.Char(
        ...
        default='[]',
        help='Domain filter: workflow only triggers for records matching this domain.'
    )
    ...
    notify_requester = fields.Boolean(
        ...
        default=True,
        help='Send email to record creator when approval status changes.'
    )
    notify_next_approver = fields.Boolean(
        string='Notify Next Approver',
        default=True,
        help='Send email to the next approver when it is their turn.'
    )
    ...
    _sql_constraints = [
        ('unique_model', 'UNIQUE(model_id)',
         'A workflow configuration already exists for this model. Only one workflow per model is allowed.')
    ]
    ...
    def _check_trigger_domain(self):
        for rec in self:
            try:
                ...
                raise ValidationError(_('Trigger Condition must be a valid Python list (Odoo domain).'))

    def action_view_requests(self):
        self.ensure_one()
        return {
            'type': 'ir.actions.act_window',
            'name': _('Approval Requests for %s') % self.name,
            'res_model': 'approval.request',
            'view_mode': 'list,form',
            'domain': [('workflow_config_id', '=', self.id)],
            'context': {'default_workflow_config_id': self.id},
        }
    ...
    def get_blocked_methods(self):
        """Return a set of method names that should be blocked."""
        self.ensure_one()
        methods = set()
        if self.block_write:
            methods.add('write')
