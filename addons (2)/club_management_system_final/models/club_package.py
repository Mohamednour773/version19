...
class ClubPackage(models.Model):
    ...
    # Registration fee flags
    is_registration_fee = fields.Boolean(
        string='Is Registration Fee',
        default=False,
        tracking=True,
        help='This is a one-time enrollment/registration fee, not a session package.',
    )
    requires_registration = fields.Boolean(
        string='Requires Registration Fee',
        default=False,
        tracking=True,
        help='Clients must hold an active registration fee membership for this branch.',
    )
    ...
    _sql_constraints = [
        ...
        ('total_sessions_positive', 'CHECK(total_sessions >= 0)', 'Total sessions cannot be negative.'),
        ('sessions_per_week_positive', 'CHECK(sessions_per_week >= 0)', 'Sessions per week cannot be negative.'),
