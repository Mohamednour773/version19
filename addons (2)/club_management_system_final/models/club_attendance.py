...
class ClubAttendance(models.Model):
    ...
    trainer_id = fields.Many2one(
        ...
        string='Primary Trainer',
        store=True,
    )
    marked_by_trainer_id = fields.Many2one(
        'club.trainer',
        string='Conducted By',
        ondelete='set null',
        help='The trainer who actually conducted this attendance. '
             'Used for commission split when a substitute replaces the primary trainer.',
    )
    ...
    def action_mark_attended(self):
        for att in self:
            ...
            # Auto-assign conducting trainer only if not already set manually.
            if not att.marked_by_trainer_id:
                session = att.session_id
                substitutes = session.trainer_line_ids.filtered(
                    lambda l: l.is_substitute and not l.is_primary
                )
                if len(substitutes) == 1:
                    # Single substitute — auto-assign
                    att.marked_by_trainer_id = substitutes[0].trainer_id.id
                else:
                    # Multiple substitutes or none — fall back to primary trainer.
                    # User must set marked_by_trainer_id manually per attendance record.
                    att.marked_by_trainer_id = session.trainer_id.id
